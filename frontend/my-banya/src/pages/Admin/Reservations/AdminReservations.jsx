...
function AdminReservations() {
  ...
  // Получаем настройки из API (время уборки и интервал бронирования)
  ...
  const timeSlots = useMemo(() => {
    const slots = [];
    const interval = bookingInterval || 30;
  ...
  const bathsToDisplay = useMemo(() => {
    ...
    return baths.map(bath => {
      ...
      const realBookings = (Array.isArray(reservations) ? reservations : []).filter(res => {
        ...
        // Сравниваем даты в локальной временной зоне
        const bookingStart = new Date(res.start_datetime);
        const bookingDate = bookingStart.toLocaleDateString('en-CA'); // YYYY-MM-DD
      ...
      realBookings.forEach(booking => {
        ...
        // Проверяем, что уборка начинается в выбранный день
        const cleaningDate = cleaningStart.toLocaleDateString('en-CA'); // YYYY-MM-DD в локальной зоне
        ...
        {bathsToDisplay.length > 1 && (
          <div className="flex space-x-1 mb-6 border-b border-gray-200">
            ...
            {bathsToDisplay.map(bath => (
              <button
                key={bath.bath_id}
                onClick={() => setActiveBathId(bath.bath_id)}
                className={`px-5 py-3 text-sm font-medium border-b-2 transition-all ${
